...
# This runner ships inside the clike repo at tests/; the .cl fixtures sit beside
# it, the shared harness tools are vendored under tests/tools/, and the example
# programs are one directory up. CLIKE_BIN defaults to the standalone build
# (./build/clike); callers can override it, e.g. the umbrella points it at
# build/bin/clike over this same corpus.
TESTS_DIR="$SCRIPT_DIR"
EX_DIR="$(cd "$SCRIPT_DIR/../examples" && pwd)"
BUILD_DIR="$SCRIPT_DIR/../build"
CLIKE_BIN="${CLIKE_BIN:-$BUILD_DIR/clike}"
CLIKE_ARGS=(--no-cache)
...
# Curl-based networking (httpsession/httprequest/dnslookup/sockets) is not an
# ext-builtin category, so it cannot be detected from --dump-ext-builtins. Probe
# it at runtime instead: a build without libcurl keeps the symbols but the stub
# prints "this build omits libcurl-based networking" and exits non-zero. Returns
# 0 when networking is compiled in. The data: URL fixtures (HttpFetchDataURL,
# UnicodeBuiltinText) need this even though they make no real network request.
has_network_support() {
    local binary="$1"
    local probe_src probe_out
    probe_src=$(mktemp "${TMPDIR:-/tmp}/clike_netprobe.XXXXXX.cl")
    printf 'int main() {\n    int s = httpsession();\n    return s < 0 ? 1 : 0;\n}\n' > "$probe_src"
    set +e
    probe_out=$("$binary" --no-cache "$probe_src" 2>&1)
    set -e
    rm -f "$probe_src"
    case "$probe_out" in
        *"omits libcurl-based networking"*) return 1 ;;
        *) return 0 ;;
    esac
}
...
run_clike_fixture() {
    local test_name="$1"
    local src="$TESTS_DIR/$test_name.cl"
    local input_file="$TESTS_DIR/$test_name.in"
    local stdout_expect="$TESTS_DIR/$test_name.out"
    local stderr_expect="$TESTS_DIR/$test_name.err"
    local disasm_expect="$TESTS_DIR/$test_name.disasm"
    local sqlite_marker="$TESTS_DIR/$test_name.sqlite"
    local net_marker="$TESTS_DIR/$test_name.net"
    ...
    # These two fixtures exercise curl-based networking (httpsession/dnslookup)
    # via in-memory data: URLs, so they have no .net server marker, yet a build
    # without libcurl cannot run them. UnicodeBuiltinText additionally uses the
    # SQLite builtins. Gate them on the detected capabilities so a minimal build
    # skips cleanly while a full build still runs them.
    if [ "$test_name" = "HttpFetchDataURL" ] && [ "$CLIKE_NET_AVAILABLE" -ne 1 ]; then
        harness_report SKIP "clike_${test_name}" "CLike fixture $test_name" "libcurl networking unavailable"
        return
    fi
    if [ "$test_name" = "UnicodeBuiltinText" ]; then
        if [ "$CLIKE_NET_AVAILABLE" -ne 1 ]; then
            harness_report SKIP "clike_${test_name}" "CLike fixture $test_name" "libcurl networking unavailable"
            return
        fi
        if [ "$CLIKE_SQLITE_AVAILABLE" -ne 1 ]; then
    ...
    if [ -f "$disasm_expect" ]; then
        ...
        # clike prints the disassembly banner as "<parent_dir>/<file>"; the
        # checked-in .disasm fixtures were captured with the source under a
        # "clike/" directory. We run from $STAGE_DIR (set below), which exposes a
        # "clike" path pointing at $TESTS_DIR, so reference the source through it
        # to reproduce the exact "clike/<name>.cl" banner the fixtures expect.
        set +e
        run_clike_command --dump-bytecode-only "clike/$test_name.cl" > /dev/null 2> "$disasm_stderr"
        local disasm_status=$?
        set -e
        strip_ansi_inplace "$disasm_stderr"
        perl -0 -pe 's/^Compilation successful.*\n//m; s/^Loaded cached bytecode.*\n//m' "$disasm_stderr" > "$disasm_stderr.clean"
        mv "$disasm_stderr.clean" "$disasm_stderr"
        rel_src="clike/$test_name.cl"
...
$diff_output")
    ...
    if [ "$status" = "PASS" ] && [ -f "$net_marker" ] && [ "${RUN_NET_TESTS:-0}" = "1" ]; then
        local server_script="$TESTS_DIR/$test_name.net"
...
if grep -q '^SDL:BOOL=ON$' "$BUILD_DIR/CMakeCache.txt" 2>/dev/null; then
    if [ "${RUN_SDL:-0}" != "1" ]; then
...
if [ ! -x "$CLIKE_BIN" ]; then
    ...
    echo "build it first: cmake -S . -B build && cmake --build build -j" >&2
    exit 1
fi
...
# A few fixtures bake in paths relative to the umbrella repo root: ImportSimple.cl
# imports "Tests/clike/imports/ImportsHelper.cl", YyjsonBuiltinTest.cl reads
# "Tests/data/yyjson_sample.json", and MStreamWords.cl reads "etc/words". The
# disassembly banner is also derived from the source's parent directory name.
# Stage a small directory that re-creates just those paths (as symlinks into the
# vendored corpus) and run from there so the fixtures resolve unchanged.
STAGE_DIR=$(mktemp -d)
mkdir -p "$STAGE_DIR/Tests"
ln -s "$TESTS_DIR" "$STAGE_DIR/clike"
ln -s "$TESTS_DIR" "$STAGE_DIR/Tests/clike"
ln -s "$TESTS_DIR/data" "$STAGE_DIR/Tests/data"
ln -s "$TESTS_DIR/etc" "$STAGE_DIR/etc"
trap 'rm -rf "$TEST_HOME" "$STAGE_DIR"' EXIT
...
if has_network_support "$CLIKE_BIN"; then
    CLIKE_NET_AVAILABLE=1
else
    CLIKE_NET_AVAILABLE=0
fi

shopt -s nullglob
for src in "$TESTS_DIR"/*.cl; do
