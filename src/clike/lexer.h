#ifndef CLIKE_LEXER_H
#define CLIKE_LEXER_H

#include <stdbool.h>

typedef enum {
    CLIKE_TOKEN_INT,
    CLIKE_TOKEN_LONG,
    CLIKE_TOKEN_LONG_LONG,
    CLIKE_TOKEN_VOID,
    CLIKE_TOKEN_FLOAT,
    CLIKE_TOKEN_DOUBLE,
    CLIKE_TOKEN_LONG_DOUBLE,
    CLIKE_TOKEN_STR,
    CLIKE_TOKEN_TEXT,
    CLIKE_TOKEN_MSTREAM,
    CLIKE_TOKEN_TASK,
    CLIKE_TOKEN_IF,
    CLIKE_TOKEN_ELSE,
    CLIKE_TOKEN_WHILE,
    CLIKE_TOKEN_FOR,
    CLIKE_TOKEN_DO,
    CLIKE_TOKEN_SWITCH,
    CLIKE_TOKEN_CASE,
    CLIKE_TOKEN_DEFAULT,
    CLIKE_TOKEN_STRUCT,
    CLIKE_TOKEN_ENUM,
    CLIKE_TOKEN_CONST,
    CLIKE_TOKEN_BREAK,
    CLIKE_TOKEN_CONTINUE,
    CLIKE_TOKEN_RETURN,
    CLIKE_TOKEN_IMPORT,
    CLIKE_TOKEN_SPAWN,
    CLIKE_TOKEN_JOIN,
    CLIKE_TOKEN_SIZEOF,
    CLIKE_TOKEN_IDENTIFIER,
    CLIKE_TOKEN_NUMBER,
    CLIKE_TOKEN_FLOAT_LITERAL,
    CLIKE_TOKEN_CHAR_LITERAL,
    CLIKE_TOKEN_CHAR,
    CLIKE_TOKEN_BYTE,
    CLIKE_TOKEN_WORD,
    CLIKE_TOKEN_UINT8,
    CLIKE_TOKEN_UINT16,
    CLIKE_TOKEN_UINT32,
    CLIKE_TOKEN_UINT64,
    CLIKE_TOKEN_STRING,
    CLIKE_TOKEN_PLUS,
    CLIKE_TOKEN_PLUS_EQUAL,
    CLIKE_TOKEN_MINUS,
    CLIKE_TOKEN_MINUS_EQUAL,
    CLIKE_TOKEN_PLUS_PLUS,
    CLIKE_TOKEN_MINUS_MINUS,
    CLIKE_TOKEN_STAR,
    CLIKE_TOKEN_STAR_EQUAL,
    CLIKE_TOKEN_SLASH,
    CLIKE_TOKEN_SLASH_EQUAL,
    CLIKE_TOKEN_PERCENT,
    CLIKE_TOKEN_PERCENT_EQUAL,
    CLIKE_TOKEN_TILDE,
    CLIKE_TOKEN_BIT_AND,
    CLIKE_TOKEN_BIT_AND_EQUAL,
    CLIKE_TOKEN_BIT_OR,
    CLIKE_TOKEN_BIT_OR_EQUAL,
    CLIKE_TOKEN_BIT_XOR,
    CLIKE_TOKEN_BIT_XOR_EQUAL,
    CLIKE_TOKEN_SHL,
    CLIKE_TOKEN_SHL_EQUAL,
    CLIKE_TOKEN_SHR,
    CLIKE_TOKEN_SHR_EQUAL,
    CLIKE_TOKEN_BANG,
    CLIKE_TOKEN_BANG_EQUAL,
    CLIKE_TOKEN_EQUAL,
    CLIKE_TOKEN_EQUAL_EQUAL,
    CLIKE_TOKEN_LESS,
    CLIKE_TOKEN_LESS_EQUAL,
    CLIKE_TOKEN_GREATER,
    CLIKE_TOKEN_GREATER_EQUAL,
    CLIKE_TOKEN_AND_AND,
    CLIKE_TOKEN_OR_OR,
    CLIKE_TOKEN_QUESTION,
    CLIKE_TOKEN_COLON,
    CLIKE_TOKEN_DOT,
    CLIKE_TOKEN_ARROW,
    CLIKE_TOKEN_SEMICOLON,
    CLIKE_TOKEN_COMMA,
    CLIKE_TOKEN_LPAREN,
    CLIKE_TOKEN_RPAREN,
    CLIKE_TOKEN_LBRACE,
    CLIKE_TOKEN_RBRACE,
    CLIKE_TOKEN_LBRACKET,
    CLIKE_TOKEN_RBRACKET,
    CLIKE_TOKEN_EOF,
    CLIKE_TOKEN_UNKNOWN
} ClikeTokenType;

typedef struct {
    ClikeTokenType type;
    const char *lexeme;
    int length;
    int line;
    int column;
    long long int_val;
    double float_val;
} ClikeToken;

typedef struct {
    const char *src;
    int pos;
    int line;
    int column;
} ClikeLexer;

void clikeInitLexer(ClikeLexer *lexer, const char *source);
ClikeToken clikeNextToken(ClikeLexer *lexer);
const char* clikeTokenTypeToString(ClikeTokenType type);

#endif
