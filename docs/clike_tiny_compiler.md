...
`tiny.clike` reads VM definitions from pscal-core's sources at compile time,
under `<root>/src/` in an installed runtime or
`<root>/components/pscal-core/src/` in a source checkout:

- opcodes: `compiler/opcodes.def`
- VM version: `core/version.h`
- type tags: `core/var_type.h`
- PSB3 container version: `core/cache.c` (`PSB3_FORMAT_VERSION`)

If `var_type.h` is missing, it falls back to canonical tags used by Tiny:

- integer: `TYPE_INT32` fallback value `2`
- string: `TYPE_STRING` fallback value `4`

An installed runtime ships no `cache.c`, so there the container version falls
back to the one the writer implements (3); the VM rejects a file of any other
version when it loads it.

Globals are slot-addressed (`DEFINE_GLOBAL_SLOT`, `GET_GSLOT`,
`GET_GSLOT_ADDRESS`), written with constant-pool name indices that the VM's
load-time link step resolves to slots. The output is a PSB3 container, and is
byte-for-byte what the Python Tiny frontend (`tools/tiny`) emits for the same
source; `Tests/run_tiny_tests.sh` checks both.
...
`tiny: opcodes.def missing ...` or `tiny: version.h missing ...`
